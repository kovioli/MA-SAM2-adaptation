...
def extract_motl_coordinates_and_score_values(
    motl: list, z_offset: int, ratio: tuple
) -> tuple:
...
def read_motl_coordinates_and_values(
    path_to_motl: str, z_offset: int, ratio: tuple
) -> tuple:
...
def precision_recall_calculator(
    predicted_coordinates, value_predicted: list, true_coordinates, radius: float
):
        ...
        for value, point in zip(value_predicted, predicted_coordinates):
            ...
            if len(close_to_point) > 0:
                ...
                if flag == "true_positive":
                    ...
                    flag == "true_positive_candidate"
                    and flag_tmp == "redundant_candidate"
                ):
            ...
            total_current_predicted_points = (
                true_positives_total + false_positives_total
            )
            precision.append(true_positives_total / total_current_predicted_points)
            recall.append(true_positives_total)
        false_negatives = [
            point for point in true_coordinates if tuple(point) not in detected_true
        ]
...
def quadrature_calculator(x_points: list, y_points: list) -> float:
    ...
    sorted_y = [p for _, p in sorted(list(zip(x_points, y_points)), key=lambda x: x[0])]
    n = len(y_points)
...
def eval_picking(
    ...
    true_values, true_coordinates = read_motl_coordinates_and_values(
        path_to_motl=gt_motl_path, z_offset=0, ratio=(1, 1)  # no need to change
    )
    ...
    print(
        f"MOTL PATH: {predicted_motl_path}\n\tauPRC = {auPRC}\n\tmax_F1 = {max_F1}\n\tfinal_F1 = {F1_score[-1]}"
    )
