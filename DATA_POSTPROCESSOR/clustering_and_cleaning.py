...
def build_tom_motive_list(list_of_peak_coordinates: list,
                          ...
                          in_tom_format=False) -> pd.DataFrame:
    """
...
def get_cluster_centroids(dataset: np.array, min_cluster_size,
                          max_cluster_size, connectivity=1) -> tuple:
...
def cluster_and_clean(
    ...
    if epoch is not None:
        original_prediction_path = os.path.join(prediction_dir, f"{tomogram_name}_epoch_{epoch:03d}.mrc")
    else:
