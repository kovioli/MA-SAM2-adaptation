...
TRAIN_DS_ID = "0001"
TRAIN_DS_TIMESTAMP = "19102024_07:00"
memory_used = False
...
PREDICTION_DIR = os.path.join("/oliver", "MA", "PREDICT", f"TS_{TRAIN_DS_TIMESTAMP}")
...
if __name__ == "__main__":
    ...
    for tomo_id in TOMO_IDs:
        ...
        pred_tomogram_info = [
            item
            for item in pred_tomogram_info_list
            if item.get("name") == f"TS_{tomo_id}"
        ][0]
        gt_motl_path = os.path.join(
            "/oliver", "MA", "GT_particle_lists", f"TS_{tomo_id}_cyto_ribosomes.csv"
        )
        # predict(
        #     PRED_ID=tomo_id,
        ...
        for th in threshold_list:
            ...
            if max_F1 > best_F1_on_th:
                ...
                break
        F1_results.append({"ds_name": f"TS_{tomo_id}", "F1": float(best_F1_on_th)})
    ...
    with open(
        os.path.join(
            ...
            "SAM2",
            "finetune_eval",
            f"{TRAIN_DS_ID}_{TRAIN_DS_TIMESTAMP}.json",
        ),
        "w",
    ) as f:
